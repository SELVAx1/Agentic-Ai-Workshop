...
class RunStore:
    ...
    def enqueue(self, thread_id: str, text: str, model: str, max_attempts: int = 3) -> str:
        """Save the user's message and create a queued run in one transaction."""
        with self.transaction() as c:
            self.append_message(thread_id, "user", text)
            run_id = str(uuid.uuid4())
            c.execute(
                "INSERT INTO run (id, thread_id, status, model, max_attempts, available_at) VALUES (?, ?, 'queued', ?, ?, ?)",
                (run_id, thread_id, model, max_attempts, self.clock()),
            )
            return run_id

    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        """Atomically claim the oldest available queued run for a worker."""
        now = self.clock()
        with self.transaction() as c:
            row = c.execute(
                """
    ...
    def heartbeat(self, run_id: str, worker_id: str, lease_seconds: float) -> bool:
        """Extend the lease only while this worker still owns the running run."""
        now = self.clock()
        cur = self.conn.execute(
    ...
    def reap_expired(self) -> list[str]:
        """Return queued runs whose leases expired, or dead-letter them if retries are exhausted."""
        now = self.clock()
        rows = self.conn.execute(
            """
        ...
        for row in rows:
            ...
            self.conn.execute(
                ...
                SET status = ?,
                    ...
                    lease_until = NULL,
                    available_at = ?,
                    finished_at = CASE WHEN ? = 'dead' THEN {NOW_SQL} ELSE finished_at END
                WHERE id = ?
                """,
                (status, available_at, status, row["id"]),
            )
        return touched
    ...
    def request_cancel(self, run_id: str) -> str | None:
        """Queue a cancellation request: immediate for queued runs, deferred for running runs."""
        with self.transaction() as c:
            ...
            if status == "queued":
                c.execute(
                    f"UPDATE run SET status = 'cancelled', cancel_requested = 0, finished_at = {NOW_SQL} WHERE id = ?",
                    (run_id,),
                )
                return "cancelled"
            if status == "running":
    ...
    def mark_cancelled(self, run_id: str, worker_id: str) -> bool:
        """Cancel a running run only if the current worker still owns the lease."""
        with self.transaction() as c:
            cur = c.execute(
                f"UPDATE run SET status = 'cancelled', lease_owner = NULL, lease_until = NULL, cancel_requested = 0, finished_at = {NOW_SQL} WHERE id = ? AND status = 'running' AND lease_owner = ?",
                (run_id, worker_id),
            )
            return cur.rowcount == 1
    ...
    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,
        ...
        """Handle a run failure according to retryability and remaining attempts."""
        with self.transaction() as c:
            row = c.execute(
            ...
            c.execute(
                f"UPDATE run SET status = ?, error_code = ?, lease_owner = NULL, lease_until = NULL,"
                f" available_at = ?, finished_at = {finished_sql} WHERE id = ?",
                (status, error_code, available_at, run_id),
            )
            return status
