...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        """Returns (application_id, created). Duplicate applications are treated as success."""
        with self.transaction() as c:
            existing = c.execute(
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        """Book only if the slot is still free and the version matches the one we read."""
        cur = self.conn.execute(
            "UPDATE interview_slot SET student_id = ?, version = version + 1 WHERE id = ? AND student_id IS NULL AND version = ?",
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        """Returns (notification_id, created). Duplicate dedupe keys are ignored."""
        with self.transaction() as c:
            existing = c.execute(
    ...
    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:
        """Run effect at most once per key, rolling back the whole transaction if it raises."""
