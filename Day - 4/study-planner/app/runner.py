...
def rebuild(store: RunStore, thread_id: str, run_id: str):
    """Rebuild the model's conversation history from the database so a resumed run continues
    exactly where it left off.

    Returns (contents, seq, pending, final_text):
      contents    full message history + every recorded step of this run
      seq         last step number used
      pending     [(seq, call_dict)] tool calls with no recorded result yet
      final_text  the model's final answer if recorded but the run was never marked complete
    """
...
def execute_run(claimed: Claimed, *, store: RunStore, db: PlannerDb, providers: dict,
                worker_id: str, lease_seconds: float,
                on_step: Callable[[dict], None] | None = None,
                record_delay: float = 0.0) -> str:
    """Drive a run to completion. Returns 'succeeded' or 'cancelled'.
    Raises AgentError or LeaseLost on error.

    record_delay widens the gap between a side effect and its database record
    — used only by the crash demo to simulate a worker dying mid-write.
    """
    run_id = claimed.run_id
    thread = store.get_thread(claimed.thread_id)
    student_id = thread["student_id"]
    system = SUPERVISOR_SYSTEM.format(student_id=student_id)
    tools = SupervisorTools(db, providers, student_id, on_step=on_step)
    ...
    # Edge case: crashed after producing the final answer but before completing the run
    ...
    while True:
        # Execute any tool calls whose results haven't been recorded yet
        for step_seq, call in pending:
            ...
            if record_delay:
                time.sleep(record_delay)  # crash drill only
        ...
        if seq >= MAX_STEPS:
            raise AgentError("step_limit",
                             f"Stopped after {MAX_STEPS} steps without an answer.", retryable=False)
        if stop := between_steps():
            return stop
