"""Talk to your Placement Assistant in the terminal. (Given.)
    ...
    python -m scripts.chat --db agent.db --thread <id>

Every tool call is printed, so you can see which tool the model chose
and what your code returned.
"""
...
def print_step(entry: dict) -> None:
    ...
    print(
        f"  {colour}\u2192 {entry['tool']}{RESET}"
        f"{DIM}({short(entry['args'], 100)}){RESET}"
    )

    print(
        f"  {DIM}\u2190 {short(entry['result'])}"
        f"  [{entry['ms']} ms]{RESET}"
    )
...
def main() -> None:
    ...
    p.add_argument(
        "--db",
        help="SQLite file for the agent's memory (Part 3)"
    )
    p.add_argument(
        "--thread",
        help="continue this thread id from --db"
    )

    a = p.parse_args()

    # Read the model name from .env.
    # If GEMINI_MODEL is not present, use gemini-2.5-flash as fallback.
    model = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")

    provider = (
        default_mock()
        if a.mock
        else GeminiProvider(model)
    )

    tools = PlacementTools(
        InMemoryPlacementRepo(),
        OutboxNotifier()
    )
    ...
    try:
        ...
        raise SystemExit(
            "ConversationStore isn't finished yet: "
            "complete Part 3.2, or run without --db."
        )

    print(
        f"Placement Assistant for {a.student} "
        f"on {provider.model}. Type 'exit' to quit.\n"
    )
    ...
    while True:
        ...
        try:
            ...
            print(
                f"{CYAN}assistant>{RESET} "
                f"{reply or '(no reply)'}\n"
            )

        except AgentError as e:
            print(
                f"{RED}{e.code}: {e.message}{RESET}\n"
            )
            ...
            print(
                f"{RED}Not implemented yet: "
                f"{where}(). Finish it, then try again.{RESET}\n"
            )
