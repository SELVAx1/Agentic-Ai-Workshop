...
class ConversationStore:
    ...
    def append_message(self, thread_id: str, role: str, text: str) -> int:
        ...
        row = self.conn.execute(
            ...
            VALUES (
                ?,
                COALESCE(
                    (SELECT MAX(seq) + 1 FROM message WHERE thread_id = ?),
                    1
                ),
                ?,
                ?
    ...
    def load_history(self, thread_id: str) -> list[dict]:
        """Return messages for a thread in sequence order."""
    ...
    def record_model_step(
        ...
        """Record a model step and add its tokens to the run atomically."""
        with self.transaction() as conn:
            step_id = conn.execute(
                """
                INSERT INTO run_step
                    (run_id, seq, kind, tokens_in, tokens_out)
                VALUES (?, ?, 'model', ?, ?)
                """,
    ...
    def record_tool_call(
        ...
        """Record a tool step and its tool call atomically."""
        with self.transaction() as conn:
            step_id = conn.execute(
                """
                INSERT INTO run_step
    ...
    def finish_run(
        ...
        """Finish a run and record its final status."""
        self.conn.execute(
            """
            UPDATE run
            SET status = ?,
    ...
    def page_messages(
        ...
        """Return one page of messages after a sequence number.

        Messages are returned in ascending sequence order. The next cursor is
        the last returned sequence number when more messages exist, otherwise
        None. No OFFSET is used, so newly arriving messages do not disturb the
        cursor.
        """
        if limit < 1:
        ...
        # Take at most `limit` messages.
        items = [dict(row) for row in rows[:limit]]

        if len(rows) > limit:
            # There are more messages. The next request starts after
            # the last message we actually returned.
            next_after = items[-1]["seq"]
        else:
            # No more messages currently exist.
            next_after = None
